...
export function SideBar() {
  ...
  return (
    <div>
      ...
      <div
        className={`fixed inset-0 ${!open && "hidden"}`}
        onClick={() => setOpen(false)}
      >
        <div
          className="absolute p-5 h-full w-72 bg-white right-0 top-0 border-l-2 border-l-accent flex flex-col gap-5"
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button onClick={() => setOpen(false)} variant="outline">
              <Link
                ...
                className="flex items-center justify-between w-full px-2"
              >
                Orders <LucidePackageCheck size={16} />
              </Link>
            </Button>
          </div>
        </div>
